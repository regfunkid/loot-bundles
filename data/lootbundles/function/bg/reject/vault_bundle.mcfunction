# This function activates when a player can't open a Vault Bundle.
#
# Inform the player they need an item to open this Loot Bundle, then play a sound and give it back to them
tellraw @s {"translate":"chat.lootbundles.vault_nokey"}
...
function lootbundles:give_item/vault_bundle {player:'@s[gamemode=!creative]',count:1}