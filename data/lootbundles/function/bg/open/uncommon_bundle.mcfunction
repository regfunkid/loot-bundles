# This function activates when a player opens a Uncommon Loot Bundle.
...
execute if score @s lootbundles.PlayerValue <= lootbundles.global lootbundles.RerollChance run function lootbundles:reroll/uncommon
...
execute if score @s lootbundles.PlayerValue > lootbundles.global lootbundles.RerollChance run execute at @s run loot spawn ~ ~ ~ loot lootbundles:uncommon_bundle
# Then, revoke the advancement so they can reactivate it later
advancement revoke @s only lootbundles:bg/open_uncommon
# The loot tables chosen can be found in the "loot_table/uncommon_bundle" file, but the catagories are: (Uncommon) Ores, Foods, Materials, and Blocks
