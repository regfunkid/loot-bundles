...
# Then, using the value that players have in the lootbundles.PlayerValue score, if that random value is equal to or less than lootbundles.CommonLootChance stored by lootbundles.global (50 by default, making it a 5% chance), give the player a Common Loot Bundle
execute if score @s lootbundles.PlayerValue <= lootbundles.global lootbundles.CommonLootChance run function lootbundles:give_item/common_bundle {player:'@s',count:1}
...
advancement revoke @s only lootbundles:bg/roll_common